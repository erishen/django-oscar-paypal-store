...
shell-sqlite: ## 进入 web 容器 (SQLite)
	docker-compose -f docker-compose.sqlite.yml exec web bash

db-shell: ## 进入数据库容器
	docker-compose exec db psql -U oscar_user -d oscar_db
...
quick-admin: ## 快速创建管理员 (默认: admin/admin123456)
	docker-compose exec web python /app/scripts/1-quick_admin.py
...
create-admin: ## 创建自定义管理员
	...
	docker-compose exec web python /app/scripts/1-quick_admin.py $(USER) $(PASS) $(EMAIL)

list-admins: ## 列出所有管理员
	docker-compose exec web python /app/scripts/1-quick_admin.py --list-only || \
	docker-compose exec web python /app/sandbox/manage.py shell -c "from django.contrib.auth import get_user_model; User = get_user_model(); admins = User.objects.filter(is_staff=True); print('=== 管理员列表 ==='); [print(f'- {u.username}: {u.email} ({\"超级用户\" if u.is_superuser else \"普通管理员\"})') for u in admins]; print(f'总计: {admins.count()} 个管理员')" 2>/dev/null || echo "请先确保服务运行: make up"

collectstatic: ## 重新收集静态文件
	docker-compose exec web python /app/sandbox/manage.py collectstatic --noinput

migrate: ## 运行数据库迁移
	docker-compose exec web python /app/sandbox/manage.py migrate --noinput

rebuild-index: ## 重建搜索索引
	docker-compose exec web python /app/scripts/6-manage_search_index.py --rebuild

update-index: ## 增量更新搜索索引
	docker-compose exec web python /app/scripts/6-manage_search_index.py --update

init-products: ## 初始化产品数据
	docker-compose exec web bash /app/scripts/2-init_products.sh
	@echo ""
	@echo "产品数据初始化完成!"
	@echo "查看统计: make check-data"

check-data: ## 查看数据统计
	docker-compose exec web python /app/scripts/3-check_data.py

check-images: ## 检查缺失图片的产品
	docker-compose exec web python /app/scripts/4-check_missing_images.py

check-images-detail: ## 检查缺失图片的产品（按分类）
	docker-compose exec web python /app/scripts/4-check_missing_images.py --category

add-placeholder-images: ## 为缺失图片的产品添加占位图（预览模式）
	docker-compose exec web python /app/scripts/5-add_default_images.py --dry-run

add-placeholder-images-10: ## 为前10个缺失图片的产品添加占位图
	docker-compose exec web python /app/scripts/5-add_default_images.py --limit 10

add-all-placeholder-images: ## 为所有缺失图片的产品添加占位图
	@echo "⚠️  此操作将为所有缺失图片的产品添加占位图"
	@docker-compose exec web python /app/scripts/5-add_default_images.py <<< "yes" || true

reset-data: ## 重置所有数据（危险操作）
	@echo "⚠️  警告：此操作将删除所有数据!"
	@read -p "确定要继续吗？(yes/no): " confirm; \
	if [ "$$confirm" = "yes" ]; then \
		docker-compose exec web python /app/sandbox/manage.py flush --noinput; \
		make init-products; \
	else \
		echo "操作已取消"; \
	fi
